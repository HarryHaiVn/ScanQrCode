...
public class CustomZXingScannerView extends View implements IViewFinder {
    ...
    private static final float DEFAULT_SQUARE_DIMENSION_RATIO = 0.8F;
    ...
    public CustomZXingScannerView(Context context) {
        super(context);
        this.mDefaultLaserColor = this.getResources().getColor(color.viewfinder_laser);
        this.mDefaultMaskColor = this.getResources().getColor(color.viewfinder_mask);
        this.mDefaultBorderColor = this.getResources().getColor(color.viewfinder_border);
    ...
    public void drawLaser(Canvas canvas) {
        ...
        this.postInvalidateDelayed(ANIMATION_DELAY, framingRect.left - POINT_SIZE, framingRect.top - POINT_SIZE, framingRect.right + POINT_SIZE, framingRect.bottom + POINT_SIZE);
    }
    ...
    public synchronized void updateFramingRect() {
        ...
        if (this.mSquareViewFinder) {
            if (orientation != Configuration.ORIENTATION_PORTRAIT) {
                height = (int) ((float) this.getHeight() * DEFAULT_SQUARE_DIMENSION_RATIO);
                width = height;
            } else {
                width = (int) ((float) this.getWidth() * DEFAULT_SQUARE_DIMENSION_RATIO);
                height = width;
            }
        ...
        if (width > this.getWidth()) {
            width = this.getWidth() - MIN_DIMENSION_DIFF;
        }

        if (height > this.getHeight()) {
            height = this.getHeight() - MIN_DIMENSION_DIFF;
        }
